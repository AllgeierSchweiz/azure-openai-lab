...
#https://www.youtube.com/watch?v=oSC2U2iuMRg&list=PLXHJCVbKXYmTsd7q_NWL30_BvdPFJoMNe&index=3&ab_channel=BugBytes
...
if st.session_state.clicked[1]:
    ...
    if user_csv is not None:
        ...
        #llm model
        llm = OpenAI(api_token = API_KEY, temperature = 0)
        ...
        #Pandas agent with imported CSV
        pandas_agent = create_pandas_dataframe_agent(llm, df, verbose = True)

        #Pandas agent with SQL Database
        #db = SQLDatabase.from_uri("sqlite:///./Chinook.db")
        #toolkit = SQLDatabaseToolkit(db=db)

        #pandas_agent = create_pandas_dataframe_agent(llm, toolkit, verbose = True)
